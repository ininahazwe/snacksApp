...
export default function Dashboard({ onProductTap }) {
  ...
  const fetchDashboard = async () => {
    ...
    // Top produits de la période. On garde product_id sur chaque entrée (et
    // pas seulement le nom) pour avoir une clé React fiable au rendu — deux
    // produits peuvent parfaitement partager le même nom.
    const compteur = {}
    activeSales.forEach(v => {
      if (!v.product_id) return
      if (!compteur[v.product_id]) compteur[v.product_id] = { id: v.product_id, nom: v.products?.name, emoji: v.products?.emoji, total: 0, qty: 0 }
    ...
    // Top clients de la période — même remarque : on garde client_id pour
    // la clé React, deux clients pouvant partager le même nom.
    ...
    activeSales.forEach(v => {
      ...
      if (!clientsCompteur[v.client_id]) {
        ...
        clientsCompteur[v.client_id] = { id: v.client_id, nom: clientName, total: 0, nb: 0 }
      }
      ...
      <div style={{ fontFamily: "'DM Sans', sans-serif" }}>
        ...
        {topProduits.length > 0 && (
            <>
              ...
              <div style={s.card}>
                {topProduits.map((p, i) => (
                    <div key={p.id} style={{ ...s.topRow, borderBottom: i < topProduits.length - 1 ? '1px solid rgba(0,0,0,0.04)' : 'none' }}>
        ...
        {topClients.length > 0 && (
            <>
              ...
              <div style={s.card}>
                {topClients.map((c, i) => (
                    <div key={c.id} style={{ ...s.topRow, borderBottom: i < topClients.length - 1 ? '1px solid rgba(0,0,0,0.04)' : 'none' }}>
                      <div style={s.topRank}>#{i + 1}</div>
