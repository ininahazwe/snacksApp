...
export default function StocksView() {
  ...
  const enregistrerReappro = async () => {
    const delta = parseInt(nouvelleQte)
    if (!delta || delta <= 0) return
    setSaving(true)

    const nouveauStock = editProduit.stock + delta
    await supabase.from('products').update({ stock: nouveauStock }).eq('id', editProduit.id)
    ...
    setProduits(ps => ps.map(p => p.id === editProduit.id ? { ...p, stock: nouveauStock } : p))
    setEditProduit(null)
    setSaving(false)
  }
          ...
          {liste.map((p, i) => {
            ...
            return (
              <div key={p.id} style={{ ...s.stockRow, borderBottom: i < liste.length - 1 ? '1px solid rgba(0,0,0,0.04)' : 'none' }}>
                ...
                <button style={s.reapproBtn} onClick={() => ouvrirReappro(p)} title="Restock">
        ...
        <div style={s.overlay} onClick={e => { if (e.target === e.currentTarget) setEditProduit(null) }}>
          <div style={s.modal}>
            ...
            <div style={s.fieldGroup}>
              <div style={s.fieldLabel}>Quantity to add</div>
              <input
                style={s.input}
                type="number"
                placeholder="Ex: 50"
...
const s = {
  ...
  reapproBtn: { width: '32px', height: '32px', borderRadius: '50%', background: '#F5F5F5', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#1A1A1A', flexShrink: 0 },
